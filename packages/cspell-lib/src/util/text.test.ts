...
describe('Util Text', () => {
    ...
    test('splits words', () => {
        ...
        expect(splitCamelCaseWord('ASCIIToUTF16')).toEqual([
            'ASCII',
            'To',
            'UTF16',
        ]);
        expect(splitCamelCaseWord('URLsAndDBAs')).toEqual([
            'Urls',
            'And',
            'Dbas',
        ]);
        expect(splitCamelCaseWord('WALKingRUNning')).toEqual([
            'Walking',
            'Running',
        ]);
    });
    ...
    test('tests breaking up text into lines', () => {
        ...
        const lines = [...Text.extractLinesOfText(sampleCode)].map(
            (m) => m.text
        );
    ...
    test('tests extractLinesOfText', () => {
        const linesA = [...Text.extractLinesOfText(sampleCode)].map(
            (m) => m.text
        );
    ...
    test('test extractText', () => {
        ...
        const results = words
            .map((wo) =>
                Text.extractText(line, wo.offset, wo.offset + wo.text.length)
            )
            .toArray();
...
describe('Validates offset conversions', () => {
    ...
    test('calculateTextDocumentOffsets', () => {
        ...
        const results = Text.calculateTextDocumentOffsets(
            'uri',
            sampleCode,
            offsets
        );
