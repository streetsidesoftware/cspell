...
describe('Validate InDocSettings', () => {
    ...
    test('tests merging inclusion and exclusion patterns into an inclusion list', () => {
        ...
        const mergedRanges = TextRange.excludeRanges(
            includeRanges,
            excludeRanges
        );
    ...
    test('tests finding matching positions', () => {
        ...
        const hereDocs = TextRange.findMatchingRanges(
            RegPat.regExPhpHereDoc,
            text
        );
    ...
    test('test sha regex', () => {
        ...
        RegPat.regExSha.lastIndex = 0;
        expect(RegPat.regExSha.test('sha1-RBT/dKUIecII7l/cgm4ywwNUnto=')).toBe(
            true
        );
        RegPat.regExSha.lastIndex = 0;
    });
...
function rangesToText(text: string, ranges: TextRange.MatchRange[]): string[] {
    const textOffsets: TextOffset[] = TextRange.extractRangeText(
        text,
        ranges
    ).map((r) => ({
        offset: r.startPos,
    ...
    return offsets.map(
        (t) =>
            `${t.row.toString().padStart(3)}:${t.col.toString().padStart(3)} ${
                t.text
            }`
    );
}
