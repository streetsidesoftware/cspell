...
describe('Validate InDocSettings', () => {
    ...
    test('tests extracting in file settings for compound words', () => {
        expect(InDoc.getInDocumentSettings('')).toEqual({
            id: 'in-doc-settings',
        });
        ...
        expect(InDoc.getInDocumentSettings(sampleText).allowCompoundWords).toBe(
            true
        );
        expect(InDoc.getInDocumentSettings(sampleCode).allowCompoundWords).toBe(
            true
        );
    });
    ...
    test('tests finding ignoreRegExp', () => {
        ...
        const regExpList = matches
            .map((s) => Text.stringToRegExp(s))
            .map((a) => (a && a.toString()) || '');
