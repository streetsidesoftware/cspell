...
export const regExPublicKey = /BEGIN\s+((?:RSA\s+)?PUBLIC)\s+KEY[\w=+\-/=\\\s]+?-*END\s+\1/g;
export const regExCert = /BEGIN\s+(CERTIFICATE|(?:RSA\s+)?(?:PRIVATE|PUBLIC)\s+KEY)[\w=+\-/=\\\s]+?-*END\s+\1/g;
