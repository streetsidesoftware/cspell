...
function normalizeSettings(
    ...
    const languageSettings = (settings.languageSettings || []).map(
        (langSetting) => ({
            ...langSetting,
            dictionaryDefinitions: normalizePathForDictDefs(
                langSetting.dictionaryDefinitions || [],
                pathToSettings
            ),
        })
    );
...
function importSettings(
    filename: string,
    defaultValues: CSpellSettings = defaultSettings
): CSpellSettings {
    ...
    Object.assign(
        finalizeSettings,
        normalizeSettings(settings, pathToSettings)
    );
    const finalizeSrc: Source = {
        name: path.basename(filename),
        ...finalizeSettings.source,
    };
...
export function readSettings(
    filename: string,
    defaultValues?: CSpellUserSettingsWithComments
): CSpellSettings {
...
export function readSettingsFiles(filenames: string[]): CSpellSettings {
    return filenames
        .map((filename) => readSettings(filename))
        .reduce((a, b) => mergeSettings(a, b), defaultSettings);
}
...
function merge(left: CSpellSettings, right: CSpellSettings): CSpellSettings {
    ...
    const includeRegExpList = takeRightThenLeft(
        left.includeRegExpList,
        right.includeRegExpList
    );
    ...
    const settings: CSpellSettingsWithSourceTrace = {
        ...
        enabledLanguageIds: replaceIfNotEmpty(
            left.enabledLanguageIds,
            right.enabledLanguageIds
        ),
        ignoreRegExpList: mergeList(
            left.ignoreRegExpList,
            right.ignoreRegExpList
        ),
        patterns: mergeList(left.patterns, right.patterns),
        dictionaryDefinitions: mergeList(
            left.dictionaryDefinitions,
            right.dictionaryDefinitions
        ),
    ...
    if (isCSpellSettingsWithSourceTrace(s)) {
        return (
            (s.source &&
                ...
                (s.source.sources[side] === ancestor ||
                    hasAncestor(s.source.sources[side], ancestor, side))) ||
            false
        );
    }
    return false;
...
export function mergeInDocSettings(
    left: CSpellSettings,
    right: CSpellSettings
): CSpellSettings {
    const merged = {
        ...mergeSettings(left, right),
        includeRegExpList: mergeList(
            left.includeRegExpList,
            right.includeRegExpList
        ),
    };
...
export function calcOverrideSettings(
    settings: CSpellSettings,
    filename: string
): CSpellSettings {
    ...
    const result = overrides
        .filter((override) =>
            checkFilenameMatchesGlob(filename, override.filename)
        )
        .reduce(
            (settings, override) => mergeSettings(settings, override),
            settings
        );
    return result;
}
...
export function finalizeSettings(settings: CSpellSettings): CSpellSettings {
    ...
    const finalized: CSpellSettingsWithSourceTrace = {
        ...settings,
        ignoreRegExpList: applyPatterns(
            settings.ignoreRegExpList,
            settings.patterns
        ),
        includeRegExpList: applyPatterns(
...
function applyPatterns(
    ...
    const patternMap = new Map(
        patternDefinitions.map((def) => [def.name.toLowerCase(), def.pattern])
    );
    ...
    const patternList = regExpList.map(
        (p) => patternMap.get(p.toString().toLowerCase()) || p
    );
...
export function checkFilenameMatchesGlob(
    filename: string,
    globs: Glob | Glob[]
): boolean {
    ...
    const matches = globs.filter((g) =>
        minimatch(filename, g, { matchBase: true })
    );
...
function mergeSources(left: CSpellSettings, right: CSpellSettings): Source {
    const {
        source: a = { name: 'left' },
    } = left as CSpellSettingsWithSourceTrace;
    const {
        source: b = { name: 'right' },
    } = right as CSpellSettingsWithSourceTrace;
    return {
...
export function getSources(
    settings: CSpellSettings | CSpellSettingsWithSourceTrace
): CSpellSettings[] {
    if (
    ...
    return right
        ? getSources(left).concat(getSources(right))
        : getSources(left);
}
