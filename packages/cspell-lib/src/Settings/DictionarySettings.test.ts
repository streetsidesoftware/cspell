...
describe('Validate DictionarySettings', () => {
    ...
    test('tests normalizing the dictionary paths', () => {
        ...
        const defs = DictSettings.normalizePathForDictDefs(
        ...
        const legacyDictionaryDefinitions = (
            dictionaryDefinitions || []
        ).map((a) => ({ ...a }));
        legacyDictionaryDefinitions[0].path = path.join('~', basePath);
        legacyDictionaryDefinitions[0].file = '';
        const tildeDefs = DictSettings.normalizePathForDictDefs(
            legacyDictionaryDefinitions!,
            '.'
        );
