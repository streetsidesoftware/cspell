...
export const languageIds: string[] = languageExtensionDefinitions.map(
    ({ id }) => id
);

let mapExtensionToLanguageIds: ExtensionToLanguageIdMap;

export function buildLanguageExtensionMap(
    defs: LanguageExtensionDefinitions
...
export function getLanguagesForExt(ext: string): string[] {
    if (!mapExtensionToLanguageIds) {
        mapExtensionToLanguageIds = buildLanguageExtensionMap(
            languageExtensionDefinitions
        );
    }
