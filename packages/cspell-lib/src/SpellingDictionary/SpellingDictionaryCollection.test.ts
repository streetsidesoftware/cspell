...
import {
    ...
    createCollectionP,
    createCollection,
} from './SpellingDictionaryCollection';
...
describe('Verify using multiple dictionaries', () => {
    ...
    test('checks for existence', async () => {
        ...
        expect(dictCollection.size).toBeGreaterThanOrEqual(
            wordsA.length - 1 + wordsB.length + wordsC.length
        );
    });

    test('checks for suggestions', async () => {
        const trie = new SpellingDictionaryFromTrie(
            Trie.Trie.create(wordsA),
            'wordsA'
        );
        const dicts = await Promise.all([
            trie,
        ...
        expect(
            sugsForTango.map((a) => a.word).filter((a) => a === 'mango')
        ).toEqual(['mango']);
    });
    ...
    test('checks for compound suggestions', async () => {
        ...
        const dictCollection = createCollection(dicts, 'test', ['Avocado']);
        const sugResult = dictCollection.suggest(
            'appletango',
            10,
    ...
    test('checks for suggestions from mixed sources', async () => {
        ...
        const dictCollection = new SpellingDictionaryCollection(
            dicts,
            'test',
            []
        );
