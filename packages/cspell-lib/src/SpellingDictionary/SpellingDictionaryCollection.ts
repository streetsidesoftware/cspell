...
export class SpellingDictionaryCollection implements SpellingDictionary {
    ...
    constructor(
        ...
        this.isDictionaryCaseSensitive = this.dictionaries.reduce(
            (a, b) => a || b.isDictionaryCaseSensitive,
            false
        );
    }
    ...
    public has(word: string, hasOptions?: HasOptions): boolean {
        ...
        return (
            ...
            isWordInAnyDictionary(this.dictionaries, word, options)
        );
    }
    ...
    public _suggest(
        ...
        const {
            numSuggestions = getDefaultSettings().numSuggestions ||
                defaultNumSuggestions,
            numChanges,
            compoundMethod,
            ignoreCase = true,
        ...
        return collector.suggestions.map((r) => ({
            ...r,
            word: r.word.replace(regexPrefix, ''),
        }));
    }
    ...
    public genSuggestions(
        ...
        this.dictionaries.forEach((dict) =>
            dict.genSuggestions(collector, _suggestOptions)
        );
    }
}
...
export function isWordInAnyDictionary(
...
export function createCollectionP(
    ...
    return Promise.all(dicts).then(
        (dicts) => new SpellingDictionaryCollection(dicts, name, wordsToFlag)
    );
}
