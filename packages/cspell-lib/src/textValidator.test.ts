...
describe('Validate textValidator functions', () => {
    ...
    test('tests ignoring words that consist of a single repeated letter', async () => {
        const dictCol = await getSpellingDictionaryCollection();
        const text =
            ' tttt gggg xxxxxxx jjjjj xxxkxxxx xxxbxxxx \n' + sampleText;
        const result = validateText(text, dictCol, {
            allowCompoundWords: true,
        });
        const errors = result
            .map((wo) => wo.text)
    ...
    test('tests case sensitive word list', async () => {
        ...
        const text =
            'We have PUBLISHed published Multiple FIXesToThePROBLEMs VeryBadProblem';
        const options = {
            allowCompoundWords: true,
            ignoreCase: false,
            flagWords,
    ...
    test('tests trailing s, ed, ing, etc.', async () => {
        ...
        const text = 'We have PUBLISHed multiple FIXesToThePROBLEMs';
        const result = validateText(text, dictWords, {
            allowCompoundWords: true,
        });
        const errors = result
            .map((wo) => wo.text)
    ...
    test('tests words crossing exclude boundaries', async () => {
        ...
        const inclusionRanges = calcTextInclusionRanges(text, {
            ignoreRegExpList: [/_/g],
        });
        ...
        expect(words.join(' ')).toBe(
            'Test the line breaks from begin to end eol'
        );
    });
...
async function getSpellingDictionaryCollection() {
    const dicts = await Promise.all([
        ...
        createSpellingDictionary(words, 'words', 'test', {
            repMap: [['’', "'"]],
        }),
    ]);
