...
const sampleFile = fsp
    .readFile(sampleFilename, 'utf8')
...
describe('Validate that Go files are correctly checked.', () => {
    ...
    test('Tests the default configuration', async () => {
        ...
        const fileSettings = cspell.combineTextAndLanguageSettings(
            settings,
            text,
            languageIds
        );
        // cspell:ignore weirdd garbbage
        const results1 = await cspell.validateText(
            'some weirdd garbbage',
            fileSettings
        );
        expect(results1.map((t) => t.text)).toEqual(
            expect.arrayContaining(['weirdd'])
        );
        expect(results1.map((t) => t.text)).toEqual(
            expect.arrayContaining(['garbbage'])
        );
