...
describe('Validate assumptions', () => {
    test('path relative', () => {
        const relCrossDevice = path.win32.relative(
            'C:\\user\\home\\project',
            'D:\\projects'
        );
        expect(relCrossDevice).toEqual('D:\\projects');
        const relSubDir = path.win32.relative(
            '/User/home/project',
            '/User/home/project/fun/with/coding'
        );
        expect(relSubDir).toBe(path.win32.normalize('fun/with/coding'));
        const relSubDirPosix = path.posix.relative(
            '/User/home/project',
            '/User/home/project/fun/with/coding'
        );
...
[path.posix, path.win32].forEach((pathInstance) => {
    describe(`Validate GlobMatcher ${
        pathInstance === path.win32 ? 'Windows' : 'Posix'
    }`, () => {
        tests().forEach(
            ([patterns, root, filename, expected, description], index) => {
                const rootPrefix = pathInstance === path.win32 ? 'C:' : '';
                root = root
                    ? pathInstance.normalize(
                          pathInstance.join(rootPrefix, root)
                      )
                    : root;
                filename = root
                    ? pathInstance.normalize(
                          pathInstance.join(rootPrefix, filename)
                      )
...
describe('Tests .gitignore file contents', () => {
    ...
    function t(filename: string, expected: boolean, comment: string) {
        test(`Test: "${comment}" File: "${filename}" (${
    ...
    t(
        root + 'src/code.test.ts',
        true,
        'Ensure that test.ts files are not allowed'
    );
    t(
        root + 'src/code.spec.ts',
        true,
        'Ensure that spec.ts files are not allowed'
    );
    t(
        '/Users/guest/code/' + 'src/code.test.ts',
        false,
        'Ensure that test files in a different root are allowed'
    );
    t(
        '/Users/guest/code/' + 'src/code.js',
        true,
        'Ensure *.js files are never allowed even in a different root.'
    );
    t(
        root + 'node_modules/cspell/code.ts',
        true,
        'Ensure that node modules are not allowed in the current root.'
    );
    t(
    ...
    tt(
        root + 'src/code.ts',
        { matched: false },
        'Ensure that .ts files are allowed'
    );
    tt(
        root + 'dist/code.ts',
...
function tests(): [string[], string | undefined, string, boolean, string][] {
    return [
        ...
        [
            ...
            '/src/.vscode/settings.json',
            false,
            'should not match nested **/.vscode',
        ],
        [
            ['**/.vscode/**'],
            undefined,
        ...
        [
            ...
            true,
            'With Root Matches nested files, *.js',
        ],
        [
            ['.vscode/'],
            '/User/code/src',
        ...
        [
            ['.vscode/'],
            '/User/code/src',
            '/User/code/src/.vscode',
            true,
            'With Root .vscode/',
        ], // This one shouldn't match, but micromatch says it should. :-(
        [
            ['.vscode/'],
            '/User/code/src',
        ...
        [
            ...
            'tests/code.test.ts',
            true,
            'Relative file with Root',
        ],
        [
            ['tests/**/*.test.ts'],
            '/User/code/src',
            'tests/nested/code.test.ts',
            true,
            'Relative file with Root',
        ],
        ...
        [
            ['{src,dist}'],
            '',
            '/User/code/distribution/settings.json',
            false,
            'Braces',
        ],
        [
