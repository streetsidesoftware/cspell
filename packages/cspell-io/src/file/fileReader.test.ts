...
describe('Validate the fileReader', () => {
    ...
    test('tests stringsToLines', async () => {
        ...
        const a = await asyncIterable.toArray(
            fReader.streamLineByLineAsync(strings)
        );
    ...
    test('test the file reader', async () => {
        const lines = await asyncIterable.toArray(
            fReader.streamFileLineByLineAsync(__filename)
        );
    ...
    test('test the lineReaderAsync', async () => {
        const lines = await asyncIterable.toArray(
            fReader.lineReaderAsync(__filename)
        );
    ...
    test('tests reading the cities sample', async () => {
        const lines = await asyncIterable.toArray(
            fReader.lineReaderAsync(fileCities)
        );
    ...
    test('tests streamFileLineByLineAsync', async () => {
        await Promise.all(
            sampleFiles.map(async (filename) => {
                const lines = await asyncIterable.toArray(
                    fReader.streamFileLineByLineAsync(filename)
                );
    ...
    test('test missing file', async () => {
        const result = asyncIterable.toArray(
            fReader.lineReaderAsync(__filename + 'not.found')
        );
        return result.then(
            () => {
