import { asyncAwait, asyncIterableToArray, filter, map, pipeAsync, pipeSync, toAsyncIterable } from './async';

describe('Validate async', () => {
    test('mergeAsyncIterables', async () => {
        const a = 'hello'.split('');
        const b = 'there'.split('');
        expect(await asyncIterableToArray(toAsyncIterable(a, b))).toEqual([...a, ...b]);
    });

    test('toAsyncIterable', async () => {
        const values = ['one', 'two', 'three'];
        expect(await asyncIterableToArray(toAsyncIterable(values, wrapInPromise(values), toAsync(values)))).toEqual(
    ...
    test('map', async () => {
        ...
        const sync = [...s];
        const async = await asyncIterableToArray(a);
