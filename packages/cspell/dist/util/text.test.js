...
describe('Util Text', () => {
    it('tests build regexp from string', () => {
        ...
        chai_1.expect(regEx4.toString()).to.be.equal('/abc/');
    });
    it('splits words', () => {
    ...
    it('splits words like HTMLInput', () => {
        return Text.extractWordsFromCodeRx('var value = HTMLInput.value;').pipe(operators_1.map(({ text }) => text), operators_1.toArray()).toPromise()
            .then(words => {
    ...
    it('tests skipping Chinese characters', () => {
        ...
        `).map(wo => wo.text).toArray()).to.deep.equal(['a', 'href', 'http', 'www', 'ctrip', 'com', 'title', 'a']);
    });
    it('tests Greek characters', () => {
        chai_1.expect(Text.extractWordsFromCode(`
            Γ γ	gamma, γάμμα
        `).map(wo => wo.text).toArray()).to.deep.equal(['Γ', 'γ', 'gamma', 'γάμμα']);
    });
    ...
    it('tests extractLinesOfTextRx', () => {
        ...
        return Text.extractLinesOfTextRx(sampleCode).pipe(operators_1.map(m => m.text), operators_1.toArray()).toPromise()
            .then(linesB => {
            chai_1.expect(linesB).to.be.deep.equal(linesA);
        });
    });
});
describe('Test the text matching functions', () => {
    it('isUpperCase', () => {
        chai_1.expect(Text.isUpperCase('first')).to.be.false;
        chai_1.expect(Text.isUpperCase('First')).to.be.false;
        chai_1.expect(Text.isUpperCase('FIRST')).to.be.true;
    });
    it('isLowerCase', () => {
        chai_1.expect(Text.isLowerCase('first')).to.be.true;
        chai_1.expect(Text.isLowerCase('First')).to.be.false;
        chai_1.expect(Text.isLowerCase('FIRST')).to.be.false;
    });
    it('isFirstCharacterUpper', () => {
        chai_1.expect(Text.isFirstCharacterUpper('first')).to.be.false;
        chai_1.expect(Text.isFirstCharacterUpper('First')).to.be.true;
        chai_1.expect(Text.isFirstCharacterUpper('FIRST')).to.be.true;
    });
    it('isFirstCharacterLower', () => {
        chai_1.expect(Text.isFirstCharacterLower('first')).to.be.true;
        chai_1.expect(Text.isFirstCharacterLower('First')).to.be.false;
        chai_1.expect(Text.isFirstCharacterLower('FIRST')).to.be.false;
    });
    // cSpell:ignore áello firstname
    it('ucFirst', () => {
        chai_1.expect(Text.ucFirst('hello')).to.be.equal('Hello');
        chai_1.expect(Text.ucFirst('Hello')).to.be.equal('Hello');
        chai_1.expect(Text.ucFirst('áello')).to.be.equal('Áello');
    });
    it('lcFirst', () => {
        chai_1.expect(Text.lcFirst('hello')).to.be.equal('hello');
        chai_1.expect(Text.lcFirst('Hello')).to.be.equal('hello');
        chai_1.expect(Text.lcFirst('áello')).to.be.equal('áello');
        chai_1.expect(Text.lcFirst('Áello')).to.be.equal('áello');
    });
    it('snakeToCamel', () => {
        chai_1.expect(Text.snakeToCamel('first')).to.be.equal('First');
        chai_1.expect(Text.snakeToCamel('firstName')).to.be.equal('FirstName');
        chai_1.expect(Text.snakeToCamel('first_name')).to.be.equal('FirstName');
        chai_1.expect(Text.snakeToCamel('FIRST_NAME')).to.be.equal('FIRSTNAME');
    });
    it('camelToSnake', () => {
        chai_1.expect(Text.camelToSnake('first')).to.be.equal('first');
        chai_1.expect(Text.camelToSnake('firstName')).to.be.equal('first_name');
        chai_1.expect(Text.camelToSnake('first_name')).to.be.equal('first_name');
        chai_1.expect(Text.camelToSnake('FIRSTName')).to.be.equal('first_name');
        chai_1.expect(Text.camelToSnake('FIRSTNAME')).to.be.equal('firstname');
    });
});
describe('Validates offset conversions', () => {
    function* getOffsets(haystack, needle) {
        let offset = -1;
        do {
            offset = haystack.indexOf(needle, offset + 1);
            if (offset > 0) {
                yield { offset, text: needle };
            }
            else {
                break;
            }
    ...
    it('calculateTextDocumentOffsets', () => {
        ...
        chai_1.expect(results[0].row).to.be.equal(6);
        chai_1.expect(results[0].doc).to.be.equal(sampleCode);
        chai_1.expect(results[0].col).to.be.equal(1);
    });
});
const sampleCode = `
/*
