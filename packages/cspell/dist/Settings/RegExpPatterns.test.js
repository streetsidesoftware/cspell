...
const RegExpPatterns_1 = require("./RegExpPatterns");
...
describe('Validate InDocSettings', () => {
    it('tests regExSpellingGuard', () => {
        ...
        chai_1.expect(m1).is.not.empty;
        chai_1.expect(m1).has.length(5);
        // cspell:disable
        chai_1.expect(m1).is.deep.equal([
    ...
    it('tests finding a set of matching positions', () => {
        ...
        chai_1.expect(ranges.length).to.be.equal(10);
    });
    it('tests merging inclusion and exclusion patterns into an inclusion list', () => {
        ...
        chai_1.expect(mergedRanges.length).to.be.equal(24);
    });
    ...
    it('tests finding matching positions', () => {
        ...
        chai_1.expect(disableChecker.length).to.be.equal(5);
    ...
    it('test matching urls', () => {
        ...
        chai_1.expect(matches).to.be.not.null;
        chai_1.expect(matches).to.have.length(1);
        chai_1.expect(matches[0]).to.be.equal('http://example.org');
    });
    it('test matching href', () => {
        ...
        chai_1.expect(matches[0]).to.be.equal('href="http://example.org"');
        chai_1.expect(matches[1]).to.be.equal('href="/example/"');
    });
    it('test sha regex', () => {
        RegPat.regExSha.lastIndex = 0;
        chai_1.expect(RegPat.regExSha.test('')).to.be.false;
        RegPat.regExSha.lastIndex = 0;
        chai_1.expect(RegPat.regExSha.test('sha512-mm6iZYQ1xbVBNsWq2VSMFuneRuO0k0wUqIT4ZfrtbD1Eb90DXmqBOPA/URyUHq6wsftxr8aXDJHTTHyyBBY95w==')).to.be.true;
        RegPat.regExSha.lastIndex = 0;
        chai_1.expect(RegPat.regExSha.test('sha512-vjiRZkhKEyZndtFOz/FtIp0CqPbgOOki8o9IcPOLTqlzcnvFLToYdERshLaI6TCz7pDWoKlmvgftqB4xlltn9g==')).to.be.true;
        RegPat.regExSha.lastIndex = 0;
        chai_1.expect(RegPat.regExSha.test('sha1-RBT/dKUIecII7l/cgm4ywwNUnto=')).to.be.true;
        RegPat.regExSha.lastIndex = 0;
    });
});
const sampleCode2 = `
