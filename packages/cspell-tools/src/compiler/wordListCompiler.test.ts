...
describe('Validate the wordListCompiler', () => {
    test('tests splitting lines', () => {
        ...
        expect(lineToWords('JPEGsBLOBs').filter(distinct()).toArray()).toEqual([
            'jpegs',
            'blobs',
        ]);
        expect(
            lineToWords('CURLs CURLing').filter(distinct()).toArray()
        ).toEqual(['curls curling', 'curls', 'curling']);
        expect(
            lineToWords('DNSTable Lookup').filter(distinct()).toArray()
        ).toEqual(['dns', 'table', 'lookup']);
        expect(lineToWords('OUTRing').filter(distinct()).toArray()).toEqual([
            'outring',
        ]);
        expect(lineToWords('OUTRings').filter(distinct()).toArray()).toEqual([
            'outrings',
        ]);
        expect(lineToWords('DIRs').filter(distinct()).toArray()).toEqual([
            'dirs',
        ]);
        expect(lineToWords('AVGAspect').filter(distinct()).toArray()).toEqual([
            'avg',
            'aspect',
        ]);
        expect(lineToWords('New York').filter(distinct()).toArray()).toEqual([
            'new york',
            'new',
            'york',
        ]);
        expect(
            lineToWords('Namespace DNSLookup').filter(distinct()).toArray()
        ).toEqual(['namespace', 'dns', 'lookup']);
        expect(
            lineToWords('well-educated').filter(distinct()).toArray()
        ).toEqual(['well', 'educated']);
        // Sadly we cannot do this one correctly
        expect(lineToWords('CURLcode').filter(distinct()).toArray()).toEqual([
            'cur',
            'lcode',
        ]);
        expect(
            lineToWords('kDNSServiceErr_BadSig').filter(distinct()).toArray()
        ).toEqual(['k', 'dns', 'service', 'err', 'bad', 'sig']);
        expect(
            lineToWords('apd_get_active_symbols').filter(distinct()).toArray()
        ).toEqual(['apd', 'get', 'active', 'symbols']);
    });

    test('test reading and normalizing a file', async () => {
        const source = await streamWordsFromFile(
            path.join(samples, 'cities.txt'),
            {}
        );
    ...
    test('tests normalized to a trie', () => {
        ...
        const tWords = [
            ...genSequence([
                normalizeWordsToTrie(genSequence(words)),
    ...
    test('test a simple hunspell dictionary depth 0', async () => {
        const source = await streamWordsFromFile(
            path.join(samples, 'hunspell', 'example.dic'),
            { maxDepth: 0 }
        );
...
describe('Validate Larger Dictionary', () => {
    test('en_US hunspell', async () => {
        ...
        const nWords = normalizeWords(genSequence(words))
            .toArray()
            .sort()
            .filter(uniqueFilter(1000));
        const results = iteratorTrieWords(trie)
            .toArray()
            .sort()
            .filter(uniqueFilter(1000));
